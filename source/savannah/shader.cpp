...
namespace Savannah
{
	...
	//								Shader
	// ----------------------------------------------------------------
	Shader* Shader::Create(const std::string& name, const std::string& vertexSrc, const std::string& fragmentSrc, const std::string& computeSrc)
	{
		return new OpenGLShader(name, vertexSrc, fragmentSrc, computeSrc);
	}
